    ...
    <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="favorites">
          <GameGrid games={favorites} viewMode={resolvedFilters.viewMode} decisions={decisions} emptyMessage="No BGG favorites match these filters yet." />
        </TabsContent>
      </Tabs>
    </div>
  )
...
function GameGrid({
  ...
  return (
    <div className={viewMode === "grid" ? "grid gap-5 md:grid-cols-2 xl:grid-cols-3" : "grid gap-5"}>
      {games.map((game) => (
        <GameCard key={game.id} game={game} viewMode={viewMode} decision={decisions[String(game.id)] as never} showPlayTime />
      ))}
    </div>
  )
}
